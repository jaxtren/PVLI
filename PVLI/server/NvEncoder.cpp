...
inline std::string NVENCstatusString(NVENCSTATUS s) {
    switch (s) {
...
void NvEncoder::CreateDefaultEncoderParams(NV_ENC_INITIALIZE_PARAMS* pIntializeParams, const VideoEncoderConfig& c)
{
    const GUID codecGuid = nvEncUtil::getNvCodec(c);
    const GUID presetGuid = nvEncUtil::getNvPreset(c);
    const NV_ENC_TUNING_INFO tuningInfo = nvEncUtil::getNvTuning(c);
    ...
    pIntializeParams->frameRateNum = c.framerate;
    ...
    //auto ke = GetCapabilityValue(codecGuid, NV_ENC_CAPS_SUPPORT_YUV444_ENCODE);

    pIntializeParams->encodeConfig->rcParams.rateControlMode = nvEncUtil::getNvRateControl(c);
    pIntializeParams->encodeConfig->rcParams.averageBitRate = c.avgBitrate;
    pIntializeParams->encodeConfig->rcParams.maxBitRate = c.maxBitrate;
    pIntializeParams->encodeConfig->rcParams.targetQuality = c.crfQuality;
    //pIntializeParams->encodeConfig->rcParams.vbvBufferSize = c.avgBitrate / c.framerate;
    pIntializeParams->encodeConfig->rcParams.enableAQ = c.adaQuant ? 1 : 0;
    //pIntializeParams->encodeConfig->rcParams.enableTemporalAQ = 1;

    if (c.multiPass)
        pIntializeParams->encodeConfig->rcParams.multiPass = NV_ENC_TWO_PASS_QUARTER_RESOLUTION;

    //pIntializeParams->encodeConfig->encodeCodecConfig.h264Config.useBFramesAsRef = NV_ENC_BFRAME_REF_MODE_DISABLED;
    //pIntializeParams->encodeConfig->encodeCodecConfig.hevcConfig.useBFramesAsRef = NV_ENC_BFRAME_REF_MODE_DISABLED;
...
int NvEncoder::GetNextInputFrameID() const
{
    return m_iToSend % m_nEncoderBuffer;
}
...
void NvEncoder::FlushEncoder()
{
    if (!m_bMotionEstimationOnly && !m_bOutputInVideoMemory)
    {
        ...
        {
            std::cout << "NvEncode error." << std::endl;
        }
    }
}
