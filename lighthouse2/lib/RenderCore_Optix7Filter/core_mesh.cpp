...
//  +-----------------------------------------------------------------------------+
void CoreMesh::UpdateAccstruc(OptixDeviceContext optixContext)
{
	...
	CHK_OPTIX( optixAccelComputeMemoryUsage( optixContext, &buildOptions, &buildInput, 1, &buildSizes ) );
	...
	{
		delete buildTemp;
		buildTemp = new CoreBuffer<uchar>( (int)buildSizes.tempSizeInBytes, ON_DEVICE );
	}
	if (buildBuffer == 0 || buildBuffer->GetSize() < (int)compactedSizeOffset)
	{
		delete buildBuffer;
	...
	if (allowCompaction)
	{
		...
		CHK_OPTIX( optixAccelBuild( optixContext, 0, &buildOptions, &buildInput, 1,
		...
		{
			CoreBuffer<uchar>* compacted = new CoreBuffer<uchar>( (int)compacted_gas_size, ON_DEVICE );
			gasData = (CUdeviceptr)compacted->DevPtr();
			CHK_OPTIX( optixAccelCompact( optixContext, 0, gasHandle, gasData, compacted_gas_size, &gasHandle ) );
			delete buildBuffer;
