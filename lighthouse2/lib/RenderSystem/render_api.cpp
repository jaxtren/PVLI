...
int RenderAPI::Render(const int* const* mask, Fragment* output, int outputSize)
{
	return renderer->GetCore()->Render( renderer->scene->camera->GetView(), mask, output, outputSize );
}
...
const vector<int>& RenderAPI::GetInstances()
{
	return renderer->GetInstances();
}
