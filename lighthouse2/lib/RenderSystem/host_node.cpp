/* host_node.cpp - Copyright 2019/2020 Utrecht University
...
bool HostNode::Update( mat4& T, vector<int>& instances, int& posInInstanceArray )
{
	...
	//update lights
	float3 pos = combinedTransform.GetTranslation();
	float3 dir = { combinedTransform[2], combinedTransform[6], combinedTransform[10] };
	dir = -normalize(dir);
	if(pointLight > -1) HostScene::pointLights[pointLight]->position = pos;
	if(spotLight > -1) {
		auto& spot = HostScene::spotLights[spotLight];
		spot->position = pos;
		spot->direction = dir;
	}
	if(directionalLight > -1) HostScene::directionalLights[directionalLight]->direction = dir;
