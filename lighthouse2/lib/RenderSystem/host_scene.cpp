...
int HostScene::AddScene( const char* sceneFile, const char* dir, const mat4& transform )
{
	...
	// load gltf file
	string cleanDir = string( dir );
	if (!cleanDir.empty() && cleanDir.back() != '/') cleanDir += '/';
	string cleanFileName = cleanDir + sceneFile;
	...
	for (size_t s = gltfModel.textures.size(), i = 0; i < s; i++)
	{
		...
		else
		{
			...
			texture->name = t;
			texture->origin = cleanDir + image.uri;
	...
	for (size_t s = gltfModel.nodes.size(), i = 0; i < s; i++)
	{
        ...
        if (gltfNode.extensions.count("KHR_lights_punctual"))
        {
            auto& l = gltfModel.lights[gltfNode.extensions["KHR_lights_punctual"].Get("light").GetNumberAsInt()];
            float3 radiance = make_float3(l.color[0], l.color[1], l.color[2]);
            radiance *= l.intensity;
            float3 pos = {0, 0, 0}, dir = {0, 0, -1}; //position and direction is updated in HostNode::Update()

            //custom extension
			float radius = 0, angle = 0;
			if(l.extras.Has("radius")) radius = l.extras.Get("radius").GetNumberAsDouble();
			if(l.extras.Has("angle")) angle = l.extras.Get("angle").GetNumberAsDouble();

            if(l.type == "point")
            {
            	newNode->pointLight = AddPointLight(pos, radiance);
				pointLights[newNode->pointLight]->radius = radius;
            }
            else if(l.type == "spot")
            {
            	newNode->spotLight = AddSpotLight(pos, dir, cos(l.spot.innerConeAngle), cos(l.spot.outerConeAngle), radiance);
				spotLights[newNode->spotLight]->radius = radius;
            }
			else if(l.type == "directional")
			{
				newNode->directionalLight = AddDirectionalLight(dir, radiance);
				directionalLights[newNode->directionalLight]->sinAngle = sin(angle);
			}
        }
