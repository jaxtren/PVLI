/* common_classes.h - Copyright 2019/2020 Utrecht University
...
struct CoreInstanceDesc
{
	...
	float LOD = 0;                              // base instance LOD, used for mipmapping
...
struct CorePointLight
{
	...
	float3 radiance; float radius;				// data1
#else
	// OpenCL float3 has 4-byte padding
	float4 position;							// w: float energy;		// data0 / position4
	float4 radiance;							// w: float radius;		// data1
#endif
	// float4 access helper
#define POINTLIGHT_ENERGY position4.w
#define POINTLIGHT_RADIUS radiance4.w
};
...
struct CoreDirectionalLight
{
	...
	float3 radiance; float sinAngle;			// data1
#else
	// OpenCL float3 has 4-byte padding
	float4 direction;							// w: float energy;		// data0 / direction4
	float4 radiance;							// w: float sinAngle;	// data1
#endif
	// float4 access helper
#define DIRLIGHT_ENERGY direction4.w
#define DIRLIGHT_ANGLE radiance4.w
};
