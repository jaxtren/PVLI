...
public:
	...
	static void compileToPTX( string& ptx, const char* cuSource, const char* sourceDir, const int cc, const int optixVer )
	{
		...
		#ifdef NDEBUG
		const char* compiler_options[] = { "-arch", versionString, "-restrict", "-std=c++11", "-default-device", "-rdc", "true", "-D__x86_64", "-use_fast_math", "-lineinfo", 0 };
        #else
		const char* compiler_options[] = { "-arch", versionString, "-restrict", "-std=c++11", "-default-device", "-rdc", "true", "-D__x86_64", "-G", 0 };
        #endif
