...
struct ShadingData
{
	...
	__device__ bool IsEmissive() const { return flags & EMISSIVE || color.x > 1.0f || color.y > 1.0f || color.z > 1.0f; }
...
{
	if (skywidth == 1) return make_float3( skyPixels[0] );
	const uint u = (uint)(skywidth * SphericalPhi( D ) * INV2PI);
	const uint v = (uint)(skyheight * SphericalTheta( D ) * INVPI);
