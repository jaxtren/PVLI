...
LH2_DEVFUNC float3 RandomPointOnHalfSphere(float3 d, float r0)
{
	float2 r = RandomInCircle(r0);
	float3 n = normalize(d);
	float3 x = normalize(cross(n, {0, 0, 1}));
	float3 y = cross(n, x);
	return x * r.x + y * r.y + n * sqrt(1.0f - r.x*r.x - r.y*r.y);
}
...
LH2_DEVFUNC float3 RandomPointOnLight( float r0, float r1, const float3& I, const float3& N, float& pickProb, float& lightPdf, float3& lightColor )
{
	...
	else if (lightIdx < (AREALIGHTCOUNT + POINTLIGHTCOUNT))
	{
		...
		float3 P = make_float3( light.data0 );	// position
		lightColor = make_float3( light.data1 );// radiance

		//radius
		const float radius = light.data1.w;
		if(radius > 0) P += RandomPointOnHalfSphere(I - P, r0) * radius;
	...
	else if (lightIdx < (AREALIGHTCOUNT + POINTLIGHTCOUNT + SPOTLIGHTCOUNT))
	{
		...
		const float4 D = light.data2;			// direction + radius
		float3 P = make_float3( V0 );

		//radius
		if(D.w > 0) P += RandomPointOnHalfSphere(I- P, r0) * D.w;
	...
	else
	{
		...
		float3 L = make_float3( light.data0 );	 // direction
		lightColor = make_float3( light.data1 ); // radiance

		//angle
		const float angle = light.data1.w;
		if(angle > 0) {
			float2 r = RandomInCircle(r0) * angle;
			float3 x = normalize(cross(L, {0, 0, 1}));
			float3 y = cross(L, x);
			L += x * r.x + y * r.y;
		}
